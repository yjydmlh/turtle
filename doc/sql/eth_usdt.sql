...
-- 创建超表语句
SELECT create_hypertable('eth_usdt', 'open_time', chunk_time_interval => INTERVAL '7 days');

-- 创建5分钟持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_5m
...
--为5分钟聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_5m',
...
-- 创建15分钟持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_15m
WITH (timescaledb.continuous) AS
...
-- 为15分钟聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_15m',
...
-- 创建一个30分钟持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_30m
WITH (timescaledb.continuous) AS
...
-- 为30分钟聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_30m',
...
-- 创建一个1小时持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_1h
WITH (timescaledb.continuous) AS
...
-- 为1小时聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_1h',
...
-- 创建一个4小时持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_4h
WITH (timescaledb.continuous) AS
...
-- 为4小时聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_4h',
  start_offset => INTERVAL '24 hours',
...
-- 创建一个1天持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_1d
WITH (timescaledb.continuous) AS
...
-- 为1天聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_1d',
...
-- 创建一个1周持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_1w
WITH (timescaledb.continuous) AS
...
-- 为1周聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_1w',
  start_offset => INTERVAL '4 weeks',
  end_offset => INTERVAL '1 week',
  schedule_interval => INTERVAL '6 hours');

-- 创建一个1个月持续聚合视图
CREATE MATERIALIZED VIEW eth_usdt_1mo
WITH (timescaledb.continuous) AS
...
-- 为1个月聚合视图添加刷新策略
SELECT add_continuous_aggregate_policy('eth_usdt_1mo',
